...
// Thanks,
// https://github.com/pvvx/esp8266web/blob/2e25559bc489487747205db2ef171d48326b32d4/app/sdklib/system/app_main.c
static void set_pll(void) {
// It is safe to assume the system is operating on a 26 MHz crystal instead of a
// 40 MHz crystal.
//	if(rom_i2c_readReg(103,4,1) != 136) { // 8: 40MHz, 136: 26MHz
// if(get_sys_const(sys_const_crystal_26m_en) == 1) { // soc_param0: 0: 40MHz,
// 1: 26MHz, 2: 24MHz
// Switch to 80 first.

// This is actually 0x88.  You can set this to 0xC8 to double-overclock the
// low-end bus.
// This can get you to a 160 MHz peripheral bus if setting it normally to 80
// MHz.
#if PERIPH_FREQ == 160
...
//	If the divisor is <9, it seems the chip cannot boot on its own, without
// reboot.
//	It looks like when divisor goes less than 5, it somehow jumps to exactly
// 5.5.
//
...
//	 0xF1: 65 MHz.
//
// Processor requires a hard reboot to link when peripheral bus is operating
// past 115 MHz
// Not sure why.
//
// ESP8285, when operating at divisors less than 6 seems to go to close to 5,
// and will boot, but never reaches 5.
...
#elif MAIN_MHZ == 80 || MAIN_MHZ == 115 || MAIN_MHZ == 160 ||                  \
    MAIN_MHZ == 173 || MAIN_MHZ == 189
  // rom_rfpll_reset();	//Reset PLL.
  set_pll(); // Set PLL
  // HWREG(DPORT_BASEADDR,0x14) &= 0x7E; //Regular clock bit.
  ets_update_cpu_frequency(80);
#elif MAIN_MHZ == 231 || MAIN_MHZ == 346 || MAIN_MHZ == 378 ||                 \
    MAIN_MHZ == 320 // Won't boot at 378 MHz.
  // rom_rfpll_reset();	//Reset PLL.
  set_pll(); // Set PLL
             // Overclock bit.
  // FIXME
...
void nosdk8266_zerobss() {
  ...
  for (addr = &_bss_start; addr < &_bss_end; addr++)
    *addr = 0; // Safe, _bss_start doesn't have to == _bss_end
}
...
void nosdk8266_init() {
  ...
  uart_div_modify(UART0, (PERIPH_FREQ * 1000000) / EMULATIONBAUD);
